...
def a_post_process(dirname, subtask_id, nums, lock):
    round, _ = [int(x) for x in subtask_id.split("_")]
...
class MapReduceTaskConfig:
    ...
    def create_subtask_list(self, round_num):
        """
        return a list of tuple (subtask_id, input_parser_fn, post_processing_fn)
        """
    ...
    def start_loop(self):
        ...
        try:
            for round in range(self.task_config.num_rounds):
                ...
                # Note we are taking the difference between x for this round and last round. Logistic regression might make more sense here
